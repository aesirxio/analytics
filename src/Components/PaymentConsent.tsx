...
import usePaymentConsent from '../utils/usePaymentConsent';

const PaymentConsent = () => {
  const { t } = useTranslation();
  const { showPaymentConsent, closePaymentConsent, handleConsent } = usePaymentConsent();

  return (
    <div
      tabIndex={-1}
      className={`toast-container position-fixed m-md-3 ${
        showPaymentConsent ? 'top-50 start-50 translate-middle' : 'bottom-0 end-0'
      }`}
    >
      <div className={`toast payment-consent ${showPaymentConsent ? 'show' : ''} custom`}>
